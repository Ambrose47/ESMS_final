...
//        String stuID = (String) lecturerProfile.get("id");
        List<Map<String, Object>> viewExamSchedule = jdbcTemplate.queryForList(
                "SELECT distinct es.slot_id,el.Date,el.Time,el.Hour FROM [ESMS_v3].[dbo].[Exam_schedule] es inner join Exam_slot el on el.id = es.slot_id where es.lecture_id is null order by es.slot_id");
